...
header-includes:
  ...
  - \usepackage{setspace}
  - \setlength{\abovecaptionskip}{-3pt} % Adjust space above captions
  - \setlength{\belowcaptionskip}{-4pt} % Adjust space below captions
  - \setlength{\textfloatsep}{-7pt} % Adjust space between text and figures
  - \usepackage{caption}
...
\vspace{-6em}
```{r scatter, echo=FALSE, fig.cap='Scatter plots with trend lines', warning=FALSE, fig.pos='H', fig.align='center', fig.margin = "1pt",fig.show='hold'}
...
```
```{r histograms_adj, echo=FALSE, warning=FALSE, fig.cap = "Univariate graphical analysis - Histograms",fig.pos='H', ,fig.align='center', out.height='50%'}
...
```{r fig:VIF1, echo=FALSE, warning=FALSE, fig.align='center', fig.width=8, fig.height=4,results='hide',fig.cap=' ', out.height="80%", fig.asp=0.5}
# Create Barplot
...
$$
\text{mortYld}_i = \beta_0 + \beta_1 X_{1i} + \beta_2 X_{2i} + \beta_3 X_{3i} + \beta_4 X_{5i} + \beta_5 X_{6i}
$$
...
$$
\text{mortYld}_i = \beta_0 + \beta_1 X_{1i} + \beta_2 X_{2i} + \beta_3 X_{3i}
$$
...
$$
\text{mortYld}_i = \beta_0 + \beta_1 X_{1i} + \beta_2 X_{2i} + \beta_3 X_{3i} + \beta_4 X_{1i}^2 + \beta_5 X_{2i}^2 + \beta_6 X_{3i}^2 
$$
A Log of the response(mortgage yield):
$$
\log(\text{mortYld}_i) = \beta_0 + \beta_1 X_{1i} + \beta_2 X_{2i} + \beta_3 X_{3i} 
$$
and lastly the log of the predictors:
$$
\text{mortYld}_i = \beta_0 + \beta_1 \log(X_{1i}) + \beta_2 \log(X_{2i}) + \beta_3 \log(X_{3i})
$$
...
```{r Model_metrics,echo=FALSE, warning=FALSE, fig.cap='Metrics for Model Comparisons'}
(p1 + p2)/
(p3 + p4)
```
